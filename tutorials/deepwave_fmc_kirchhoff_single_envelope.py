...
over a three-layer medium (water / aluminum / water) containing a **single
centred air void**, using the :mod:`deepwave` finite-difference wave solver. The
...
# ---------------------------------------------------------------------------
# 1. Build the three-layer model with a single air void (ultrasonic NDT scale)
# ---------------------------------------------------------------------------
...
# a single air void, centred under the array in the middle of the aluminum
# layer. The void is ~1 aluminum wavelength across at 5 MHz (alu lambda ~ 1.26
# mm), i.e. a small (sub-wavelength-ish) point-like scatterer.
void_r = 0.63e-3  # ~0.63 mm radius -> ~1.26 mm diameter ~ 1 alu wavelength
centers = [(0.020, 0.015)]  # x = 20 mm (array centre), z = 15 mm (mid aluminum)
...
# (b2) Representative A-scan signals (individual time traces) from the sim.
# Use the central shot (the element sitting over the void) and a few receivers
# at increasing source-receiver offset, plus the pulse-echo trace shown as
# total vs scattered field so the weak void echo becomes visible.
zarr = elem_z[0]


def event_time(cz):
    """Vertical two-way time from the array to depth cz (pulse-echo), incl. the
    source peak delay -- a guide for the main specular events."""
    twt = 2.0 * ((Z_TOP - zarr) / V_WATER + max(cz - Z_TOP, 0.0) / V_ALU)
    return (twt + peak_time) * 1e6  # [us]


t_us = t * 1e6
cz0 = centers[0][1]
events = {"front wall": event_time(Z_TOP), "void": event_time(cz0),
          "back wall": event_time(Z_BOT)}
rec_offsets_mm = [0, 4, 8, 15]
rec_idx = [min(ishot + int(o), nr - 1) for o in rec_offsets_mm]
...
# right: pulse-echo trace -- total vs scattered (different scales)
ax = axs[1]
pe_tot = fmc[ishot, ishot]
pe_sct = fmc_scat[ishot, ishot]
for name, te in events.items():
    ax.axvline(te, color="gray", ls="--", lw=0.8)
